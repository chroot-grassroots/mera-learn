...
COMPONENT_PATTERNS = {
    'component_class': r'export\s+class\s+(\w+)\s+extends\s+BaseComponent',
...
def scan_component_file(filepath: Path) -> Optional[Dict[str, str]]:
    ...
    if all(field in component_info for field in required_fields):
        component_info['file'] = filepath.stem  # filename without extension
...
def scan_yaml_files_in_directory(directory: str, file_type: str) -> List[Dict[str, str]]:
...
def parse_yaml_lessons() -> Tuple[List[Dict], Set[int], Set[int], Dict[int, int]]:
    ...
    lesson_ids = set()
    component_ids = set()
    domain_lesson_map = {}  # domainId -> [lessonIds]
...
def parse_curriculum() -> Optional[Dict]:
    ...
    # Look for curriculum.yaml or first .yaml file
...
def generate_component_registry(components: List[Dict], lessons: List[Dict], 
                                lesson_ids: Set, component_ids: Set,
                                domain_lesson_map: Dict[int, List[int]],
                                curriculum: Optional[Dict], domains: List[Dict],
                                menus: List[Dict]) -> str:
    """Generate complete TypeScript registry with all 11 mappings."""
    
    # Generate component imports and registrations
    imports = []
    registrations = []
 ...
 */
export const curriculumData = {json.dumps(curriculum, indent=2) if curriculum else 'null'};
...
def main():
    ...
    # Phase 2: YAML File Discovery (for runtime loading)
    print("\n📂 Phase 2: YAML File Discovery")
    yaml_files = scan_all_yaml_files()
    
    # Phase 3: YAML Content Parsing (for mappings)
    ...
    component_registry = generate_component_registry(
        components, lessons, lesson_ids, component_ids,
        domain_lesson_map, curriculum, domains, menus
    )
