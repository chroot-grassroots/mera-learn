...
export async function initializationOrchestrator(): Promise<void> {
  ...
  const lessonConfigs: Map<number, ParsedLessonData> = new Map(
    Array.from(rawLessons.entries()).map(([id, lesson]) => {
      // Flatten components and ADD page index to each
      const components = lesson.pages.flatMap((page, pageIndex) =>
  ...
  if (loadResult === null) {
    ...
    showCriticalError({
      ...
      message:
        "Cannot proceed - user authentication state is invalid. This indicates a critical bootstrap failure.",
      technicalDetails:
        "progressLoader returned null despite bootstrap authentication check",
    });
    throw new Error("Authentication failure after bootstrap - critical error");
  }

  // Check if bundle is null (new user with no backups)
  if (!loadResult.bundle) {
    console.log("📝 New user detected - creating default bundle");

    const { MeraBridge } = await import("../solid/meraBridge.js");
    const webId = MeraBridge.getInstance().getWebId()!;

    // Use enforceDataIntegrity with empty string to trigger default bundle creation
    const { enforceDataIntegrity } = await import("./progressIntegrity.js");
    const defaultResult = enforceDataIntegrity("", webId, lessonConfigs);

    // Fix the webId field (it's set to error value by default)
...
async function handleRecoveryScenario(
  ...
  if (
    scenario === RecoveryScenario.DEFAULT_FAILED_RECOVERY &&
    sessionFileExists
  ) {
    return await handleCatastrophicLoss();
  }
  ...
  if (
    scenario === RecoveryScenario.DEFAULT_WEBID_MISMATCH &&
    sessionFileExists
  ) {
    return await handleCatastrophicLoss();
  }
 ...
 */
async function handleDegradedRecovery(
  metrics: ProgressLoadResult["recoveryMetrics"],
): Promise<boolean> {
  ...
  const issuesSummary =
    issues.length > 0 ? issues.join(", ") : "Some data required recovery";
